...
def get_project_root():
    ...
    try:
        ...
        root = os.environ.get('PYTHONPATH', '/opt/render/project/src')
        if not os.path.exists(root):
            # Fallback to current directory
            root = os.path.dirname(os.path.abspath(__file__))
        logger.info(f"Project root directory: {root}")
        return root
...
def generate_model():
    ...
    try:
        ...
        model_path = os.path.join(root, 'models', 'fake_news_model.joblib')
        vectorizer_path = os.path.join(root, 'models', 'tfidf_vectorizer.joblib')
...
def verify_setup():
    ...
    root = get_project_root()
    required_files = [
        os.path.join(root, 'models', 'fake_news_model.joblib'),
        os.path.join(root, 'models', 'tfidf_vectorizer.joblib'),
